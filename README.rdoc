...
A more complicated example:
  ...
  factory :user, {
    :name       => "user name",
    :login      => "$UNIQ(7)",                      # the $UNIQ(n) magic variable interpolates to a unique string of length n
    :email      => "user_$COUNT@email.com",         # the $COUNT magic variable interpolates to an incremental number, beginning with 1
...
There are two magic variables: $UNIQ(), and $COUNT. See the comments in the above example for more info.
Another available helper method is increment!(key), which works like $COUNT, but returns an integer and can be assigned an arbitrary key that is unrelated to the model.
